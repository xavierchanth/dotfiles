...
let
  ...
  clientTemplateText = ''
    # Merge this provider into the selected Codex login home's config.toml.
  ...
  bootstrap = pkgs.writeShellApplication {
    name = "cliproxyapi-oauth-bootstrap";
    runtimeInputs = [ pkgs.coreutils pkgs.systemd pkgs.util-linux ];
    text = ''
      set -euo pipefail
      umask 077
      if [ "$(id -u)" -ne 0 ]; then
        echo 'cliproxyapi-oauth-bootstrap must run as root' >&2
        exit 1
      fi
      restart=0
      ...
      runuser --user ${serviceUser} -- ${package}/bin/cli-proxy-api \
        -config ${runtimeConfig} -codex-device-login -no-browser
    '';
  };
...
in
{
  options.dotfiles.cliproxyapi = {
    ...
    canonicalBaseUrl = lib.mkOption {
      type = lib.types.str;
      readOnly = true;
      default = "https://codex.lab.xavierchanth.xyz/v1";
      description = "Future tailnet-only Codex client base URL";
    };
    ...
    clientTemplate = lib.mkOption {
      ...
      description = "Secret-free Codex custom-provider configuration template";
    };
  };
  ...
  config = {
    ...
    systemd.services.cliproxyapi = {
      description = "Third-party Codex subscription proxy";
