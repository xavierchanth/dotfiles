...
assert unit.environment.USAGE_DATA_DIR == "/data";
assert unit.environment.USAGE_DB_PATH == "/data/usage.sqlite";
assert unit.environment.CPA_MANAGER_DATA_KEY_PATH == "/data/data.key";
assert unit.environment.CPA_MANAGER_ADMIN_KEY_FILE == "/data/admin-key";
...
assert unit.serviceConfig.BindPaths == [ "/var/lib/cpa-manager-plus:/data" ];
assert builtins.length unit.serviceConfig.ExecStartPre == 2;
