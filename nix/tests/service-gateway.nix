...
let
  ...
  diagramsHostname = "excalidraw.lab.xavierchanth.xyz";
  ...
  diagramsRoute = gateway.routes.${diagramsHostname};
  ...
  diagramsVhost = hades.services.caddy.virtualHosts.${diagramsHostname};
...
assert builtins.attrNames gateway.routes == [ cliProxyApiHostname cpampHostname diagramsHostname executorHostname homepageHostname ];
...
assert diagramsRoute.upstream.address == "127.0.0.1" && diagramsRoute.upstream.port == 3100;
assert diagramsRoute.healthPath == "/" && diagramsRoute.upstreamUnit == "excalidraw.service";
...
assert diagramsVhost.hostName == diagramsHostname && diagramsVhost.listenAddresses == [ "127.0.0.1" ];
...
assert lib.all (vhost: lib.hasInfix "tls internal" vhost.extraConfig) [ homepageVhost executorVhost diagramsVhost cliProxyApiVhost cpampVhost ];
...
assert lib.hasInfix "reverse_proxy http://127.0.0.1:3100" diagramsVhost.extraConfig;
assert lib.hasInfix "health_uri /" diagramsVhost.extraConfig;
...
assert builtins.elem "excalidraw.service" caddy.wants;
assert builtins.elem "excalidraw.service" caddy.after;
