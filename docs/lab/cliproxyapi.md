...
CLIProxyAPI is a third-party compatibility proxy that can use a ChatGPT/Codex
subscription as an upstream credential and present a Responses-compatible API
to Codex clients. This is not an OpenAI-supported subscription gateway. OpenAI
can change the upstream protocol, authentication flow, product behavior, or
terms independently; treat this service as experimental and revocable.
...
`codex.lab.xavierchanth.xyz`: models, Responses streaming, Responses WebSocket
...
1. On Hades, run `sudo cliproxyapi-oauth-bootstrap` from an interactive terminal.
2. Open the displayed OpenAI device URL on Xavier's trusted browser and enter
   the one-time device code. The helper does not put that code in argv or a
   service log.
...
The device flow avoids a browser callback listener. If browser callback login
is ever used for diagnosis, upstream defaults to loopback port `1455`; tunnel
that port explicitly and never add it to Caddy or the firewall.

## Client enrollment and revocation

The secret-free template is installed at
`/etc/cliproxyapi/codex-client.toml`. Merge it into the selected Codex login
home on Poseidon or Zeus. Retrieve only that client's token from a trusted
Hades terminal:
...
client uses `https://codex.lab.xavierchanth.xyz/v1`, the Responses wire API,
...
3. A Codex Responses request streams to completion over HTTP and a separate
   request completes through the Responses WebSocket transport.
