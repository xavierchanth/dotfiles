...
Workspace policy:
- After implementation, leave the working copy as it is and ask Xavier whether to handle version-control work inline, hand it to an available Version Control Team, or leave it for later. An existing Team does not itself authorize a handoff or mutation. For requested work, use the [Team scope guidance](../../orchestration/coordination-core/references/team-scope.md) and coordinate access to shared state.
