...
Workspace policy:
- When a repository has an established Version Control Team, route shared history mutations through that owner under the [Team scope guidance](../../orchestration/coordination-core/references/team-scope.md). Contributors continue editing, testing, and read-only inspection, and send the owner change manifests, readiness, and evidence. Coordinate any in-flight mutation before transferring responsibility; the role does not expand authorization.
- For collaborative JJ repositories, prefer the existing shared checkout as the accumulation surface because it keeps the stream of edits available for semantic sorting. When independent branch lifecycles, risky experiments, conflicting dependency states, or filesystem isolation would materially help, recommend a separate workspace and briefly explain the benefit and tradeoff. Create or select a separate Codex worktree or JJ workspace only after Xavier explicitly agrees; carry that approval forward within its agreed scope.
...
- Once approved, create additional JJ workspaces under .jj/workspaces/ with clear task-oriented directory names, e.g. jj workspace add .jj/workspaces/fix-login --name fix-login.
